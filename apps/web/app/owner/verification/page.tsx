...
function Content({
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `http://localhost:3001/api/v1/owner/self/verified-requests?listingId=${listingId}&listingType=${listingType}`,
          {
            method: "GET",
            headers: {
  ...
  const handleSubmit = async () => {
    ...
    if (existingRequest) {
      ...
      try {
        const response = await fetch(`http://localhost:3001/api/v1/owner/self/owner-update-verification/${existingRequest.id}`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            token: token,
      ...
      if (selectedOption === 'self') {
        ...
        try {
          const response = await fetch('http://localhost:3001/api/v1/agent/verification-request', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              token: token,
