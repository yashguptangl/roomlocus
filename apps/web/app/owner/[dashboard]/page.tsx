...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      async function ownerDeatils() {
        try {
          const response = await axios.get(
            `http://localhost:3001/api/v1/owner/details-owner`,
            { headers: { token } }
          );
      ...
      async function usedLeadData(token: string, ownerId: string) {
        try {
          const leadResponse = await axios.get(
            `http://localhost:3001/api/v1/owner/contact-logs/${ownerId}`,
            { headers: { token } }
          );
      ...
      async function fetchImagesForListing(type: string, listingId: string) {
        try {
          const res = await axios.get(
            `http://localhost:3001/api/v1/owner/images/${type}/${listingId}`,
            { headers: { token } }
          );
      ...
      const fetchListings = async (token: string, ownerId: string) => {
        try {
          const response = await axios.get(
            `http://localhost:3001/api/v1/owner/${ownerId}/listings`,
            { headers: { token } }
          );
          ...
          const dayNightListings = data.listings.RoomDayNight?.map((item: ListingItem) => ({
            ...item,
            type: "daynightroom",
            listingId: item.id,
          })) || [];
      
          const allListings = [...flatListings, ...pgListings, ...roomListings, ...dayNightListings];
  ...
  const toggleButton = async (listingId: string, type: string, isVisible: boolean) => {
    try {
      ...
      const response = await axios.post(
        "http://localhost:3001/api/v1/owner/publish",
  ...
  const handleDeleteListing = async (listingId: string, type: string) => {
    try {
      ...
      const response = await axios.delete("http://localhost:3001/api/v1/owner/listing", {
        headers: { token },
        data: { listingId, type },
      });
          ...
          <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50">
            <div className="bg-white p-4 rounded-md shadow-md w-9/12 sm:w-2/3 md:w-1/3 lg:w-1/4">
              ...
              <div className="space-y-4 text-center flex flex-col">
                ...
                <Link className="text-blue-500 hover:underline" href="/owner/daynightroom">
                  DAY NIGHT ROOM
                </Link>
              </div>
