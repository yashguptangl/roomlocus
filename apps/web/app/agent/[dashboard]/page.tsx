      ...
      const getProgress = async () => {
        try {
          const response = await fetch(
            `http://localhost:3001/api/v1/agent/progress/${decodedToken.agentId}`,
            {
              method: "GET",
              headers: {
                token: token,
          ...
          const agentData = await axios.get(
            `http://localhost:3001/api/v1/agent/agent/${decodedToken.agentId}`,
            {
              headers: {
                token: token,
                "Content-Type": "application/json",
      ...
      if (decodedToken) {
        try {
          const response = await axios.get(
            `http://localhost:3001/api/v1/agent/agent-requests/${decodedToken.agentId}`,
            {
              headers: {
                token: token,
              },
  ...
  useEffect(() => {
    const fetchVerifiedRequests = async () => {
      if (decodedToken) {
        try {
          const response = await axios.get(
            `http://localhost:3001/api/v1/agent/agent-verified-properties/${decodedToken.agentId}?sort=${sortOrder}`,
            {
              headers: {
                token: token,
              },
